...
def run_exp(dataset_name, model_name, dd_type,
    ...
    class_per_peer, samples_per_class, rate_unbalance, alpha, source_class, target_class, resume, aggregator=None):
    print('\n--> Starting experiment...')
    ...
    rate_unbalance = rate_unbalance, alpha = alpha, source_class = source_class, target_class= target_class, num_attackers=int(num_peers * attackers_ratio),aggregator=aggregator)   # 传递全局多组目标类)
    ...
    flEnv.run_experiment(attack_type = attack_type, malicious_behavior_rate = malicious_behavior_rate, 
                    ...
                    rule=rule, resume = resume,aggregator=None)
    print('\n--> End of Experiment.')
