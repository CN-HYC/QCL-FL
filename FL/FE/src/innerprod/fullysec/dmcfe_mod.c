...
#include "cifer/innerprod/fullysec/dmcfe_mod.h"
#include "cifer/internal/dlog.h"
#include "cifer/internal/hash.h"
#include "cifer/sample/uniform.h"

void mod_cfe_dmcfe_client_init(mod_cfe_dmcfe_client *c, size_t idx) {
    ...
    char root0[randombytes_SEEDBYTES];
    octet tmp_root0 = {0, sizeof(root0), root0};
    ECP2_BN254 P;
    ECP2_BN254_mapit(&P, &tmp_root0);
    ECP2_BN254_copy(&c->mask[0], &P);
    char root1[randombytes_SEEDBYTES];
    octet tmp_root1 = {0, sizeof(root1), root1};
    ECP2_BN254_mapit(&P, &tmp_root1);
    ECP2_BN254_copy(&c->mask[1], &P);//每个客户生成随机ECP2_BN254掩码mask二维向量
...
//释放客户端内存
void mod_cfe_dmcfe_client_free(mod_cfe_dmcfe_client *c) {
    mpz_clear(c->order);
    //cfe_mat_free(&(c->share));
    cfe_vec_free(&(c->s));
}

//mask_sum聚合每个用户的部分解密密钥掩码
void mod_cfe_dmcfe_mask_sum(mod_cfe_dmcfe_client *c, ECP2_BN254 *masksum[2], size_t num_clients) {
    /*
    cfe_mat add;
    ...
    */
    ...
    ECP2_BN254_add(masksum[0], &(c->mask[0]));//累加每个用户的mask值
    ECP2_BN254_add(masksum[1], &(c->mask[1]));
}

void mod_cfe_dmcfe_encrypt(ECP_BN254 *cipher, mod_cfe_dmcfe_client *c, mpz_t x, char *label, size_t label_len, char *label_mod) {
    ECP_BN254 h;
    BIG_256_56 tmp_big;
    ...
    BIG_256_56 tmp_big2;
    mpz_t hash_result;//计算hash标签L2
    mpz_init(hash_result);
    octet tmp_str = {0, 32, malloc(32)};
    octet tmp_hash = {0, sizeof(label_mod), label_mod};
    mhashit(SHA256, -1, &tmp_str, &tmp_hash);//计算标签L2的hash
    mpz_import(hash_result, tmp_str.len, 1, 1, 1, 0, tmp_str.val);
    BIG_256_56_from_mpz(tmp_big2, hash_result);//转换类型然后相乘
    ...
    for (int i = 0; i < 2; i++) {//计算哈希标签H（0 label）和H（1 label）
        ...
        ECP_BN254_mul(&h, tmp_big2);//再乘上标签L2的hash
...
void mod_cfe_dmcfe_fe_key_part_init(cfe_vec_G2 *key_share) {
    cfe_vec_G2_init(key_share, 2);
}

void mod_cfe_dmcfe_derive_fe_key_part(cfe_vec_G2 *fe_key_part, mod_cfe_dmcfe_client *c, cfe_vec *y, char *label) {
    /*
    ...
    */
    ...
    octet tmp_str = {0, 32, malloc(32)};;
    octet tmp_hash = {0, sizeof(label), label};
    mhashit(SHA256, -1, &tmp_str, &tmp_hash);//计算标签L2的hash
    mpz_import(hash_result, tmp_str.len, 1, 1, 1, 0, tmp_str.val);
    BIG_256_56_from_mpz(tmp_big2, hash_result);//转换类型然后相乘
    ...
    for (size_t k = 0; k < 2; k++) {
        ...
        */
        ...
        ECP2_BN254_add(&(fe_key_part->vec[k]), &h);//部分解密密钥vec[0]=g^(yi*s0) mod p vec[1]=g^(yi*s1) mod p

        ECP2_BN254_mul(&(fe_key_part->vec[k]), tmp_big2);//部分解密密钥乘以hash标签L2
        ECP2_BN254_add(&(fe_key_part->vec[k]), &c->mask[k]);//部分解密密钥添加mask掩码 vec[0]=g^(yi*s0 + mask[0]) mod p
        
    }                                                         
    mpz_clear(tmp);
}

cfe_error mod_cfe_dmcfe_decrypt(mpz_t res, ECP_BN254 *ciphers, cfe_vec_G2 *key_shares,
                            char *label, size_t label_len, cfe_vec *y, mpz_t bound,ECP2_BN254 *mask_sum[2]) {
    cfe_vec_G2 keys_sum;
    ...
    ECP2_BN254_sub(&(keys_sum.vec[0]), (mask_sum[0])); //减去掩码和
    ECP2_BN254_sub(&(keys_sum.vec[1]), (mask_sum[1]));
