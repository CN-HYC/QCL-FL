...
link it with the `-lcifer` flag when building your code.
	...
	$ gcc -o example example.c -I/usr/local/include -L/usr/local/lib -lgmp -lcifer
	./example

gcc example_dmcfe_mod.c /home/nicoisspy/cifer/src/innerprod/fullysec/dmcfe_mod.c -o example_dmcfe_mod -L/usr/local/lib -lgmp -lcifer -lsodium  链接mod.c文件 在example路径下输入
链接问题最终在tasks.json中 -g下面添加dmcfe_mod.c解决
