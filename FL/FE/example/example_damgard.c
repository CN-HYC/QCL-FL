...
//CDG+18基于的IPFE，算法详见论文分享24-7-17 P11
...
int main() {
    ...
    cfe_vec x;
    cfe_vec_init(&x, l);
    printf("Enter %zu integers between 0 and %lu for vector x:\n", l, mpz_get_ui(bound));
    ...
    // the encrypted x; we get it from user input
    cfe_vec y;
    cfe_vec_init(&y, l);
    printf("Enter %zu integers between 0 and %lu for vector y:\n", l, mpz_get_ui(bound));
    for (size_t i = 0; i < l; ++i) {
        unsigned long val;
        scanf("%lu", &val);
        mpz_t tmp;
