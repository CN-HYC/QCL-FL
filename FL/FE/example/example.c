...
int main() {
    ...
    size_t l = 5; // dimension of encryption vector 加密向量维度
    mpz_t bound; // bound of the input values set to 2^10  输入上界
    mpz_init(bound);
    mpz_set_ui(bound, 2);
    mpz_pow_ui(bound, bound, 10);
    // modulus_len defines the security of the scheme, the higher the better
    // for precomputed parameters it needs to be in {1024, 1536, 2048, 2560, 3072, 4096}  模数长度
